...
/* ---------------- login ---------------- */
const EYE_SHOW = '<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7Z" fill="none" stroke="currentColor" stroke-width="1.8"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="1.8"/>';
const EYE_HIDE = '<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7Z" fill="none" stroke="currentColor" stroke-width="1.8"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="1.8"/><line x1="3" y1="21" x2="21" y2="3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>';
...
$('togglePass').addEventListener('click', () => {
  ...
  $('eyeIcon').innerHTML = show ? EYE_HIDE : EYE_SHOW;
  $('togglePass').setAttribute('aria-label', show ? 'Ocultar senha' : 'Mostrar senha');
  inp.focus();
});

// Caps Lock — avisa antes do erro acontecer, não depois
['keydown','keyup'].forEach(ev => {
  $('pass').addEventListener(ev, e => {
...
$('loginForm').addEventListener('submit', e => {
  e.preventDefault();
  if(logging) return;                 // evita duplo submit
